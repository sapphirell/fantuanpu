...
class UserController extends Controller
{
    ...
    public function user_center(Request $request)
    {
        ...
        $this->data['user_avatar'] = config(
                'app.online_url'
            ) . \App\Http\Controllers\User\UserHelperController::GetAvatarUrl($this->data['uid']);

        /**
         * 用户私信
         */
        $this->data['letter'] = PmListsModel::where('min_max', 'like', $this->data['uid'] . "\_%")->orwhere(
            'min_max',
            'like',
            "%\_" . $this->data['uid']
        )
    ...
    public function read_letter(Request $request)
    {
        ...
        if ($request->input('to_uid'))
        {
            ...
            $min_max = $uid < $request->input('to_uid') ? $uid . "_" . $request->input('to_uid') : $request->input(
                    'to_uid'
                ) . "_" . $uid;
    ...
    public function user_friends(Request $request)
    {
        ...
        {
            $value['favatar'] = config(
                    'app.online_url'
                ) . \App\Http\Controllers\User\UserHelperController::GetAvatarUrl($value['fuid']);
        }
    ...
    public function user_view(Request $request)
    {
        ...
        $this->data['user']->avatar = config(
                'app.online_url'
            ) . \App\Http\Controllers\User\UserHelperController::GetAvatarUrl($this->data['user']->uid);
        $this->data['user_count']   = CommonMemberCount::GetUserCoin($request->input('uid'));
        //查询用户最近发的帖子
        $this->data['thread'] = ForumThreadModel::where('authorid', $request->input('uid'))
            ->orderBy('dateline', 'desc')
            ->paginate(15)
            ->toArray()['data'];
    ...
    public function add_my_like(Request $request)
    {
        ...
        $uid = Redis::get(CoreController::USER_TOKEN['key'] . $request->input('token'));
        //是否已经有记录了
        $haslike = MyLikeModel::where('uid', $uid)
            ->where('like_id', $request->input('like_id'))
            ->where('like_type', 1)
            ->first();
        if ($haslike->id)
        {
    ...
    public function complete_action(Request $request)
    {
        ...
        $act = ActionController::complete_action(
            $request->input('act'),
            $request->input('uid'),
            $request->input('rid')
        );
