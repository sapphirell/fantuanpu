...
class SukiClockModel extends Model
{
    ...
    {
        $data = self::select(DB::raw("cid,sum(clock_money) as sum,count(cid) as count,alert_type,clock_name,clock_date,clock_end,date_format(clock_date,'%Y·%m') as ym"))->where(['uid'=>$uid]);
        switch ($group)
        {
            case false :
